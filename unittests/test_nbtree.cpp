...
BOOST_AUTO_TEST_CASE(Test_nbtree_superblock_commit) {
    // Test plan
    // Fill superblock with data
    // Commit first time
    // Commit second time
    // Check that two identical blocks was written to disk
    // Add more data
    // Commit third time
    // Check that the third block was added and it's different (contains more data)
    auto bstore = BlockStoreBuilder::create_memstore();
    IOVecSuperblock sblock(42, EMPTY_ADDR, 4, 2);

    const auto btype = NBTreeBlockType::LEAF;
    ...
    for (const auto& ref: refs) {
        auto status = sblock.append(ref);
        BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);
    }

    aku_Status status;
    LogicAddr addr1, addr2, addr3, addr4;
    std::tie(status, addr1) = sblock.commit(bstore);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);
    std::tie(status, addr2) = sblock.commit(bstore);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);

    std::shared_ptr<Block> block1, block2, block3, block4;
    std::tie(status, block1) = bstore->read_block(addr1);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);
    std::tie(status, block2) = bstore->read_block(addr2);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);

    BOOST_REQUIRE_EQUAL(block1->get_size(), block2->get_size());
    BOOST_REQUIRE_EQUAL(memcmp(block1->get_cdata(), block2->get_cdata(), block1->get_size()), 0);
    ...
    std::tie(status, addr3) = sblock.commit(bstore);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);

    std::tie(status, block3) = bstore->read_block(addr3);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);

    BOOST_REQUIRE_EQUAL(block2->get_size(), block3->get_size());
    BOOST_REQUIRE_NE(memcmp(block2->get_cdata(), block3->get_cdata(), block2->get_size()), 0);

    IOVecSuperblock sblock2(addr3, bstore, true);
    std::tie(status, addr4) = sblock.commit(bstore);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);

    std::tie(status, block4) = bstore->read_block(addr4);
    BOOST_REQUIRE_EQUAL(status, AKU_SUCCESS);

    BOOST_REQUIRE_EQUAL(block1->get_size(), block4->get_size());
    BOOST_REQUIRE_NE(memcmp(block1->get_cdata(), block4->get_cdata(), block1->get_size()), 0);
}

                                         ...
                                         std::vector<double>     *output)
{
    ...
    for (auto i = 0u; i < numvalues; i++) {
        ...
        // ///////////////////////////////////////////// //
        // SimpleFcmStreamWriter & SimpleFcmStreamReader //
        // ///////////////////////////////////////////// //
...
bool SimpleFcmStreamWriter::tput(double const* values, size_t n) {
    assert(n % 2 == 0);
    // this method doesn't modifies nelements_j because it always writes
    // in 2^K sized chunks
    for (size_t i = 0; i < n; i+=2) {
        u64 prev_diff, curr_diff;
...
std::tuple<u64, unsigned char> SimpleFcmStreamWriter::encode(double value) {
    union {
        double real;
        u64 bits;
    } curr = {};
...
bool SimpleFcmStreamWriter::put(double value) {
    u64 diff;
    u8 flag;
    std::tie(diff, flag) = encode(value);
    if (nelements_ % 2 == 0) {
...
size_t SimpleFcmStreamWriter::size() const { return stream_.size(); }

bool SimpleFcmStreamWriter::commit() {
    if (nelements_ % 2 != 0) {
        // `input` contains odd number of values so we should use
        // empty second value that will take one byte in output
...
//  //////////////////////  //
//  Simple2FcmStreamWriter  //
//  //////////////////////  //

Simple2FcmStreamWriter::Simple2FcmStreamWriter(Base128StreamWriter& stream)
    : stream_(stream)
    , predictor_(PREDICTOR_N)
    , prev_diff_(0)
    , nelements_(0)
...
bool Simple2FcmStreamWriter::tput(double const* values, size_t n) {
    assert(n % 2 == 0);
    // this method doesn't modifies nelements_j because it always writes
    // in 2^K sized chunks
    for (size_t i = 0; i < n; i+=2) {
...
size_t Simple2FcmStreamWriter::size() const { return stream_.size(); }

bool Simple2FcmStreamWriter::commit() {
    if (nelements_ % 2 != 0) {
        // `input` contains odd number of values so we should use
        // empty second value that will take one byte in output
        if (!stream_.put<u64>(prev_diff_)) {
...
SimpleFcmStreamReader::SimpleFcmStreamReader(Base128StreamReader& stream)
    : stream_(stream)
    , predictor_(PREDICTOR_N)
    , flags_(0)
    , iter_(0)
...
double SimpleFcmStreamReader::next() {
    unsigned char flag = 0;
    if (iter_++ % 2 == 0) {
        flags_ = static_cast<u32>(stream_.read_raw<u8>());
        flag = static_cast<unsigned char>(flags_ >> 4);
...
const u8 *SimpleFcmStreamReader::pos() const { return stream_.pos(); }
